...
const fs = require('fs');
const { fileDomain, filePath } = require('../constant');
...
router.get('/:slug/live', async (req, res) => {
    ...
    try {
        const m3u8File = fs.readFileSync(`${filePath}/video/${link}/master.m3u8`, 'utf-8');
        ...
        try {
            ...
            const files = fs.readdirSync(`${filePath}/video/${link}`)
            ...
            sortedFiles.forEach(file => {
                const tsFilePath = `${fileDomain}/api/segment/${link}/${file}`;
...
router.get('/:slug/:name', async (req, res, next) => {
    ...
    try {
        const m3u8File = fs.readFileSync(`${filePath}/video/${link}/${name}`, 'utf-8');
        const m3u8Content = m3u8File.toString('utf-8');
        const extinfLines = m3u8Content.match(/#EXTINF:[\d.]+,/g);
